(
	...
	(:predicates
		...
		(usat ?Habitacio - Habitacio) ; Habitacio amb alguna reserva
	)
	...
	(:action assigna
		...
		:effect (and
			...
			(increase (cost-reserves) (*(*(totalReserves) (totalReserves)) 3))
			)
			(when
				(not (usat ?habitacio))
				(and
					(usat ?habitacio)
					(increase (cost-reserves) (*(totalReserves) 3))
				)
			)
			(increase (cost-reserves) (- (personesAdmeses ?habitacio) (personesReserva ?reserva)))
			(evaluat ?reserva)
			(ready ?reservaNext)
	...
	(:action no-assignat
		...
		:effect (and
			(increase (cost-reserves) (*(*(totalReserves) (*(totalReserves) (totalReserves))) 3))
